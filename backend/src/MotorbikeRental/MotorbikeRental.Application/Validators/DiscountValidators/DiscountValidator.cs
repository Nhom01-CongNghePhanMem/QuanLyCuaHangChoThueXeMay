...
namespace MotorbikeRental.Application.Validators.DiscountValidators
{
    public class DiscountValidator : IDiscountValidator
    {
        ...
        public DiscountValidator(ICategoryRepository categoryRepository)
        {
        ...
        public async Task<(bool, List<string>)> ValidateForCreate(DiscountCreateDto discountCreateDto, CancellationToken cancellationToken = default)
        {
            ...
            List<Category> category = (await categoryRepository.GetCategoriesNoTracking(cancellationToken)).ToList();
        ...
        {
            List<string> errors = new List<string>();
            List<Category> categories = (await categoryRepository.GetCategoriesNoTracking(cancellationToken)).ToList();
