...
namespace MotorbikeRental.Domain.Interfaces.IRepositories.IPricingRepositories
{
    ...
    {
        Task<(IEnumerable<Discount>, int)> GetFilterData(string? search, int pageNumber, int pageSize, DateTime? filterStartDate, DateTime? filterEndDate, bool? IsActive, CancellationToken cancellationToken = default);
